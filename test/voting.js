...
contract('Voting', (accounts) => {
  ...
  beforeEach('Deploys contract', async () => {
    ...
    votingInstance = await Voting.new('Title', 'Description', { from: accounts[0] });
  });
  ...
  it('Can vote', async () => {
    ...
    (prevCount.toNumber() + 1).should.equal(afterCount.toNumber(), 'Count is not raised');
  });

  it('Owner can announce result', async () => {
    const transaction = await votingInstance.announceResult({ from: accounts[0] });
