...
contract('Voting', (accounts) => {
  ...
  it('Stores owner, title and description', async () => {
    ...
    assert.equal(storedDescription, 'This is voting 101', 'Description does not match');
  ...
  it('Can vote', async () => {
    ...
    assert.equal(prevState.count.toNumber() + 1, afterState.count.toNumber(), 'Count is not raised');
  });
  ...
  it('Owner can announce result', async () => {
    ...
    assert.equal(log.args.winnerOption.count, 1, 'Winner vote count does not match');
